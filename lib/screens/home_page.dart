...
class _HomePageState extends State<HomePage> {
  ...
  @override
  void dispose() {
    _scrollController.dispose(); _searchFocusNode.dispose(); _searchController.dispose();
    super.dispose();
  }
      ...
      body: CustomScrollView(
        ...
        slivers: [
          if (!_isSearchActive)
            SliverToBoxAdapter(
              child: Container(
                padding: EdgeInsets.fromLTRB(16, statusBarHeight + 12, 16, 4), // Reduced bottom padding
                decoration: BoxDecoration(
                  gradient: LinearGradient(colors: [topBarTopColor, topBarBottomColor], begin: Alignment.topCenter, end: Alignment.bottomCenter),
                ),
                child: Row(
          ...
          if (!_isSearchActive)
            SliverToBoxAdapter(
              child: Container(
                ...
                margin: const EdgeInsets.fromLTRB(16, 0, 16, 16), // Reduced top margin from 16 to 8
                decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), color: const Color(0xFFF5E6D3)),
                child: const Center(child: Text('Featured Product', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold))),
              ),
            ),
            ...
            sliver: _isSearchActive
                ...
                : SliverGrid(
                    ...
                    gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                      crossAxisCount: 2, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 0.63,
                    ),
                  ),
          ),
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, height: 1.2), maxLines: 2, overflow: TextOverflow.ellipsis),
                  const SizedBox(height: 8),
                  Text(product['price']!, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
                  Text(product['oldPrice']!, style: const TextStyle(decoration: TextDecoration.lineThrough, color: Colors.grey, fontSize: 11)),
                  ...
                  qty == 0 
                    ? GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: const Center(child: Text('Add', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13))),
                        ),
                      )
                    : Container(
                        height: 34, width: double.infinity,
                        ...
                        child: Row(
                          ...
                          children: [
                            IconButton(padding: EdgeInsets.zero, icon: const Icon(Icons.remove, size: 18), onPressed: () => setState(() { if (qty > 0) _cartQuantities[name] = qty - 1; })),
                            Text('$qty', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                            IconButton(padding: EdgeInsets.zero, icon: const Icon(Icons.add, size: 18), onPressed: () => setState(() => _cartQuantities[name] = qty + 1)),
                          ],
                        ),
                      ),
                ],
...
class _SearchBarDelegate extends SliverPersistentHeaderDelegate {
  final double scrollOffset; final String selectedCategory; final bool isSearchActive;
  final FocusNode focusNode; final TextEditingController searchController;
  final Function(String) onCategoryTap; final Function(String) onSearchChanged;
  final Function(String) onSearchSubmitted; final VoidCallback onCancelSearch;

  _SearchBarDelegate({
    required this.scrollOffset, required this.selectedCategory, required this.isSearchActive,
    required this.focusNode, required this.searchController, required this.onCategoryTap,
    required this.onSearchChanged, required this.onSearchSubmitted, required this.onCancelSearch,
  });

  @override
  double get minExtent {
    final statusBarHeight = MediaQueryData.fromView(WidgetsBinding.instance.platformDispatcher.views.first).padding.top;
    return (isSearchActive ? 60 : 95) + statusBarHeight; // Reduced from 65/110
  }
  ...
  Widget build(BuildContext context, double shrinkOffset, bool overlapsContent) {
    final double statusBarHeight = MediaQuery.of(context).padding.top;
    double opacity = (scrollOffset / 80).clamp(0, 1); // Faster transition
    Color bgColor = Color.lerp(const Color(0xFFD2B48C), Colors.white, opacity)!;
    final double topPadding = isSearchActive ? statusBarHeight + 4 : 4 + (statusBarHeight * (shrinkOffset / 20).clamp(0, 1));

    return Container(
      decoration: BoxDecoration(gradient: LinearGradient(colors: [bgColor, Colors.white], begin: Alignment.topCenter, end: Alignment.bottomCenter)),
            ...
            children: [
              Expanded(
                child: Container(
                  ...
                  child: TextField(
                    controller: searchController, focusNode: focusNode,
                    onChanged: onSearchChanged, onSubmitted: onSearchSubmitted,
                    onTap: () => focusNode.requestFocus(),
                    cursorColor: Colors.black, showCursor: true, 
                    decoration: const InputDecoration(
                      hintText: 'Search products', prefixIcon: Icon(Icons.search, color: Colors.grey, size: 20),
              ...
              if (isSearchActive)
                TextButton(onPressed: onCancelSearch, child: const Text('Cancel', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold))),
            ],
          ),
          if (!isSearchActive) ...[
            const SizedBox(height: 8),
            SizedBox(
              ...
              child: ListView(
                ...
                children: ['All', 'Skincare', 'Haircare', 'Supplements', 'Beauty'].map((cat) {
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Center(child: Text(cat, style: TextStyle(color: isSelected ? Colors.white : Colors.black, fontSize: 12))),
                    ),
                  );
                }).toList(),
              ),
